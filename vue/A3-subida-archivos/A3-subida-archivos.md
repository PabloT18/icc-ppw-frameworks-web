...
La subida de archivos (file upload) es una funcionalidad esencial en aplicaciones modernas: imágenes de perfil, documentos, imágenes de producto. Este módulo cubre el flujo completo desde la selección del archivo hasta su envío al servidor, con previsualización, validación y retroalimentación de progreso.
...
### Envío con `FormData` y progreso simulado
...
```typescript
async function simularSubida(): Promise<void> {
  enviando.value = true
  progreso.value = 0
