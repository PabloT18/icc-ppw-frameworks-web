...
1. **Captura de pantalla del Login funcionando**
  ...
  ![Login funcionando.](./assets/11_login!.png)
  **Descripción de la Imagen:** Pagina de login Funcionando

2. **Captura de Firebase Console → Authentication** con usuario registrado

  ![Pagina de Registrar funcionando.](./assets/11_registrar.png)
  **Descripción de la Imagen:** Pagina de registrar Funcionando con el usuario recien ingresando

3. **Captura de Firebase Console → Firestore** con documentos de favoritos

  ![Firebase con los favoritos funcionando.](./assets/11_favoritos.png)
  **Descripción de la Imagen:** Pagina firebase con los favoritos agregados!

4. **Captura de la aplicación mostrando:**
   - Lista de personajes de Simpsons
    
  ![Pagina final funcionando.](./assets/11_funcionando.png)
  **Descripción de la Imagen:** Pagina simpson funcionando con los favoritos agregados.

5. **Enlace al repositorio de GitHub** con el código

Reposiorio:
https://github.com/Juan0Fernandez/icc-ppw-03-ui-fundamentos

6. **Enlace al repositorio de GitHub PAGES** con el código

Git pages
https://juan0fernandez.github.io/icc-ppw-03-ui-fundamentos/

7. **Texto de reflexión** (mínimo 200 palabras) sobre:
   - Diferencias entre autenticación y autorización
   - Ventajas de usar Firebase vs backend propio
   - Cómo funcionan los guards en Angular
   Reflexión sobre la Práctica 11: Autenticación y Persistencia

Al abordar la Práctica 11, que se centró en la integración de Angular con Firebase para manejar la autenticación y la persistencia de datos (favoritos), el proceso fue una demostración clara de cómo se construyen las aplicaciones modernas con un fuerte enfoque en el backend sin servidor (serverless).

1. Diferencia entre Autenticación y Autorización

Para mí, la diferencia esencial entre Autenticación y Autorización es el paso lógico que siguen. La Autenticación es la primera pregunta: ¿Quién eres? Es el proceso de verificar la identidad de un usuario, lo que logramos con éxito al implementar el formulario de Login y Registro (Email/Password) en Firebase Authentication.

La Autorización, en cambio, es la segunda pregunta: ¿Qué puedes hacer? Es la capa de permisos. En esta práctica, la Autorización se hizo evidente y crítica en las Reglas de Seguridad de Firestore. No solo verificamos quién era el usuario (autenticación), sino que forzamos la regla de que solo el dueño de un favorito (el userId) pudiera leer o modificar ese documento específico, protegiendo así los datos de otros usuarios.

2. Ventajas de Usar Firebase vs. Backend Propio

La ventaja de usar Firebase, especialmente Cloud Firestore, sobre construir un backend propio (con, por ejemplo, Node.js y MongoDB), es inmensa para el desarrollo rápido:

Velocidad de Desarrollo: No tuvimos que escribir ni mantener servidores, ni endpoints REST. Simplemente inyectamos servicios de AngularFire y llamamos a funciones como addDoc o getDocs.

Escalabilidad Instantánea: Firebase escala automáticamente. Si nuestra aplicación pasa de 10 a 10,000 usuarios, la infraestructura de autenticación y la base de datos se adaptan sin intervención manual.

Seguridad Delegada: Delegamos la gestión de contraseñas y las complejidades del protocolo de autenticación (OAuth, tokens) a Google, lo que es mucho más seguro que intentar implementarlo uno mismo.

3. Funcionamiento de los Guards en Angular

Los Guards resultaron ser la columna vertebral para la Autorización en el frontend. Los Guards (authGuard y publicGuard) son simplemente funciones que se ejecutan antes de que una ruta se active.

El authGuard usa el estado de la autenticación (authService.user$) para decidir: si existe un usuario logueado, devuelve true (permite el paso); si no, devuelve false y redirige a /login.

El publicGuard hace lo opuesto, asegurando que si ya estoy logueado, no pueda regresar a las páginas de Login o Registro, mejorando la experiencia del usuario.

En conclusión, esta práctica me enseñó que la integración de Firebase permite enfocarse en la lógica del frontend y la experiencia del usuario, confiando en las robustas herramientas de la nube para manejar las complejidades del acceso y la persistencia de datos de forma segura y eficiente.
