...
<img src="fire1.jpg" alt="router"> 

2. **Captura de Firebase Console → Authentication** con usuario registrado

<img src="fire2.jpg" alt="router"> 

3. **Captura de Firebase Console → Firestore** con documentos de favoritos

<img src="fire3.jpg" alt="router"> 

4. **Captura de la aplicación mostrando:**

<img src="fire4.jpg" alt="router"> 

   - Lista de personajes de Simpsons
5. **Enlace al repositorio de GitHub** con el código

https://github.com/diegotorres006/03-componentes.git 

6. **Enlace al repositorio de GitHub PAGES** con el código

https://diegotorres006.github.io/03-componentes/#/login 

7. **Texto de reflexión** (mínimo 200 palabras) sobre:

Reflexión sobre Seguridad, Backend y Arquitectura en Angular

Diferencias entre Autenticación y Autorización

Aunque a menudo se usan indistintamente, autenticación y autorización son dos procesos de seguridad distintos pero complementarios. La autenticación responde a la pregunta "¿Quién eres?"; es el proceso de verificar la identidad de un usuario, como lo hicimos en la práctica al implementar el Login con correo/contraseña o Google. Si las credenciales son válidas, el sistema confirma la identidad.

Por otro lado, la autorización responde a "¿Qué puedes hacer?" o "¿A dónde puedes entrar?". Una vez que el usuario está autenticado, la autorización determina sus privilegios. En nuestra aplicación, los Guards (authGuard) actúan como agentes de autorización, permitiendo el acceso a la ruta de "Simpsons" solo si el usuario tiene permiso (estar logueado), mientras que publicGuard autoriza el acceso al Login solo a quienes no lo están.

Ventajas de usar Firebase vs. Backend Propio

Utilizar Firebase como Backend-as-a-Service (BaaS) ofrece ventajas significativas en términos de velocidad y mantenimiento frente a desarrollar un backend propio (con Node.js, Python o Java). La principal ventaja es la abstracción de la infraestructura: no necesitamos configurar servidores, gestionar bases de datos, ni preocuparnos por parches de seguridad del servidor; Firebase maneja todo eso.

Además, la integración con AngularFire acelera el desarrollo drásticamente. Implementar un sistema de autenticación seguro desde cero puede tomar días o semanas, mientras que con Firebase Authentication lo logramos en cuestión de horas. También destaca la capacidad de tiempo real de Firestore, que actualiza la interfaz automáticamente sin necesidad de recargar la página, algo complejo de implementar manualmente con WebSockets. Sin embargo, un backend propio ofrece más control y flexibilidad para lógica de negocio muy compleja o cuando se requiere no depender de un proveedor específico.

Funcionamiento de los Guards en Angular

Los Guards en Angular son servicios que actúan como "porteros" o interceptores en el ciclo de vida de la navegación del Router. Antes de que Angular cargue un componente asociado a una ruta, ejecuta los guards configurados (como CanActivate).

Estos guards ejecutan una lógica booleana o devuelven un UrlTree: si el resultado es true, la navegación continúa y el usuario ve la página; si es false o una redirección, la navegación se cancela o se redirige a otra ruta (como al Login). En nuestra práctica, utilizamos Observables y Signals dentro de los guards para esperar la respuesta asíncrona de Firebase (authState), asegurando que la decisión de permitir el paso se base en el estado real de la sesión del usuario, garantizando así una seguridad robusta en el lado del cliente.
